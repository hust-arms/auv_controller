...
class auvCSVParser():
    def __init__(self, date):
        ...
        self.ts_ind_ = 0
        self.rpm_ind_ = 30
        # index for model with front fins
        self.fin0_ind_ = 37
        self.fin1_ind_ = 36
        self.fin2_ind_ = 38
        self.fin3_ind_ = 34
        ...
        with open(csv_fn) as f:
            ...
            for row in f_csv:
                if not(k == 0):
                    ...
                    param.fin0_ = float(row[self.fin0_ind_])
                    param.fin1_ = float(row[self.fin1_ind_])
                    param.fin2_ = float(row[self.fin2_ind_])
                    param.fin3_ = float(row[self.fin3_ind_])
                    param.fin4_ = float(row[self.fin4_ind_])
                    param.fin5_ = float(row[self.fin5_ind_])
                    self.param_list_.append(param)

                k += 1

class auvOutlineControl():
    def __init__(self, auv_name, csv_fn, date):
        # initialize
        ...
        self.auv_csv_parser_ = auvCSVParser(date)
